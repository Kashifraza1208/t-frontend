...
const StoreReviwesCard = ({ items }) => {
	...
	return (
		<div className="relative">
					...
					return (
						<div
							...
							<div className="gap-2.5 flex w-full mt-5">
								<Image
									src={userId?.profilePic || '/images/man.png'}
									alt={userId?.name}
									width={300}
									height={300}
			...
			<button
				className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-700 text-white p-2 rounded-r-md cursor-pointer"
				onClick={() => handleScroll('left')}>
				{'<'}
			</button>
			<button
				className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-700 text-white p-2 rounded-l-md cursor-pointer"
