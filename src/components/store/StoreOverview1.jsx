...
const CircularCardGrid = ({ category }) => {
	...
	return (
		<div className="flex flex-col  mb-5">
			<div className="text-2xl  px-2 font-semibold lg:block">
				{category.name}
			</div>
			<div className="flex mt-3 flex-row items-start justify-between w-full gap-2 overflow-auto lg:gap-4 grid-container">
				{categoriesData?.length > 0 &&
					categoriesData.map((cat) => (
						<Link
							key={cat._id}
							href={`/subcategory/[subcategory_id]`}
							as={`/subcategory/${cat._id}`}>
							<div>
					...
					<>
						...
						<div className="flex flex-row items-center justify-between w-full gap-4 overflow-auto grid-container">
							{Array.from({ length: 5 }).map((_, index) => {
								const productIndex =
									(currentPosition + index) % products.length;
