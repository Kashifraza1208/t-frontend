...
const SearchBox = () => {
    ...
    const [popularSearches, setPopularSearches] = useState([
        'shirt', 'jeans', 'smartwatch', 'ipad', 'samsung', 'iphone'
    ]);
    ...
    const handleKeyDown = (e) => {
        if (e.key === 'Enter') {
            if (selectedResultIndex >= 0 && selectedResultIndex < searchResults.length) {
                handleResultClick(searchResults[selectedResultIndex]._id, searchResults[selectedResultIndex].name);
    ...
    return (
        <div className="flex flex-row items-center justify-center order-4 lg:order-2 w-full">
            <div className="relative rounded flex items-center justify-between w-full font-poppins pl-4 pr-2 py-1 border border-gray-200 lg:w-[400px]">
                ...
                {showSuggestions && (
                    <div ref={suggestionRef} className="right-0.5 bg-white w-full p-4 flex items-start justify-center flex-col z-[100] absolute top-12">
                        ...
                        {uniqueCategories.map((category, index) => (
                            ...
                            >
                                <IoSearch size={24} />
                                <span className="ml-2" style={{ color: '#FAAC06' }}>in {category.name}</span>
                            </div>
                        ))}
                    </div>
                )}
                    ...
                    <div className="right-0.5 bg-white w-full p-4 flex items-start justify-center flex-col z-[100] absolute top-12 ">
                        {searchResults.map((result, index) => {
                            return (
                                <div
                                    key={index}
                                    className={`cursor-pointer border-b w-full py-2 flex items-center ${selectedResultIndex === index ? 'bg-gray-200' : ''}`}
